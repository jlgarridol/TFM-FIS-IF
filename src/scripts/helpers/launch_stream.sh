...
docker exec -d fis-hubu-consumer-$ID /bin/bash -c "cd app && /root/miniconda3/envs/default-conda/bin/python consumer.py --kafkahost='kafka_kafka_1:29092' --output=/mnt/data --topic=video-stream-patient-$ID --sparkhost='spark://spark-master-fishubu:7077' $OPTIONSCONSUMER"
