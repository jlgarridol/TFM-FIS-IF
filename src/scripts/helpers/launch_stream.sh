...
docker exec -d fis-hubu-consumer-$ID /bin/bash -c "cd app && python consumer.py --kafkahost='kafka_kafka_1:29092' --output=/mnt --topic=video-stream-patient-$ID --sparkhost='spark://spark-master-fishubu:7077' $OPTIONSCONSUMER"
